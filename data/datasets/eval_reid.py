...
def eval_func_gpu(distmat, q_pids, g_pids, q_camids, g_camids, max_rank=50):     # [3368, 15913], [3368,], [15913,]
    """Evaluation with market1501 metric
        Key: for each query identity, its gallery images from the same camera view are discarded.
        """
    num_q, num_g = distmat.shape
    if num_g < max_rank:
        max_rank = num_g
        print("Note: number of gallery samples is quite small, got {}".format(num_g))
    # indices = torch.argsort(distmat, dim=1)   # [3368, 15913]
    # matches = (g_pids[indices] == q_pids[:, np.newaxis]).byte()    # [3368, 15913]
    ...
    for q_idx in range(num_q):       # 3368
        ...
        indices = torch.argsort(distmat[q_idx])  # [123138, 15913]
        matches = (g_pids[indices] == q_pid).float()  # [3368, 15913]

        # # remove gallery samples that have the same pid and camid with query
        # remove = (g_pids[indices] == q_pid) & (g_camids[indices] == q_camid)      # [15913,]
        # keep = np.invert(remove)      # [15913,]
        keep = (g_pids[indices] != q_pid) | (g_camids[indices] != q_camid)

        # compute cmc curve
        # binary vector, positions with value 1 are correct matches
        orig_cmc = matches[keep]       # [15908,]
        if orig_cmc.sum().item() <= 0:
            # this condition is true when query identity does not appear in gallery
            continue

        cmc = orig_cmc.cumsum(dim=0)          # [15908,]
        cmc[cmc > 1] = 1

        all_cmc.append(cmc[:max_rank].cpu().data.numpy().tolist())
        num_valid_q += 1.

        # compute average precision
        # reference: https://en.wikipedia.org/wiki/Evaluation_measures_(information_retrieval)#Average_precision
        num_rel = orig_cmc.sum()         # 14
        tmp_cmc = orig_cmc.cumsum(dim=0)      # [15908,], [0,0,0,...,14,14,14]
        tmp_cmc = [x / (i + 1.) for i, x in enumerate(tmp_cmc)]     # [15908,]
        tmp_cmc = torch.tensor(tmp_cmc).cuda() * orig_cmc    # [15908,]
        AP = tmp_cmc.sum() / num_rel
...
def eval_func_small(distmat, q_pids, g_pids, q_camids, g_camids, max_rank=50):     # [3368, 15913], [3368,], [15913,]
    """Evaluation with market1501 metric
        Key: for each query identity, its gallery images from the same camera view are discarded.
        """
    num_q, num_g = distmat.shape
    if num_g < max_rank:
        ...
        print("Note: number of gallery samples is quite small, got {}".format(num_g))
    # indices = np.argsort(distmat, axis=1)   # [3368, 15913]
    # matches = (g_pids[indices].astype(np.int32) == q_pids[:, np.newaxis].astype(np.int32)).astype(np.int32)    # [3368, 15913]
    ...
    for q_idx in range(num_q):       # 3368
        ...
        indices = np.argsort(distmat[q_idx])  # [3368, 15913]
        matches = (g_pids[indices] == q_pid).astype(np.int32)  # [3368, 15913]
...
def eval_func_part(distmat, distmat_head, distmat_upper, distmat_lower, q_pids, g_pids, q_camids, g_camids, max_rank=50):     # [3368, 15913], [3368,], [15913,]
    """Evaluation with market1501 metric
        Key: for each query identity, its gallery images from the same camera view are discarded.
        """
    dist_mat = 0.4 * (distmat + distmat_head + distmat_upper + distmat_lower)

    num_q, num_g = dist_mat.shape
    if num_g < max_rank:
        max_rank = num_g
        print("Note: number of gallery samples is quite small, got {}".format(num_g))
    indices = np.argsort(dist_mat, axis=1)   # [3368, 15913]
    idx_rank = np.zeros_like(indices)
    for ii in range(indices.shape[1]):
        idx_rank[:, indices[:, ii]] = ii

    mat_dif = abs(distmat_upper - distmat_lower)
    # indices_dif = np.argsort(mat_dif, axis=1)[:, ::-1]
...
def eval_func_var(distmat, distmat_head, distmat_upper, distmat_lower, q_pids, g_pids, q_camids, g_camids, max_rank=50):     # [3368, 15913], [3368,], [15913,]
    """Evaluation with market1501 metric
        Key: for each query identity, its gallery images from the same camera view are discarded.
        """
    dist_mat = 0.4 * (distmat + distmat_head + distmat_upper + distmat_lower)
