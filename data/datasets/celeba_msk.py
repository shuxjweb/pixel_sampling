...
class CELEBA_MSK(BaseDataset):
    ...
    def __init__(self, root='data', verbose=True, **kwargs):
        super(CELEBA_MSK, self).__init__()
        ...
        self.query = self._process_dir(self.query_dir, os.path.join(self.mask_dir, 'query'), relabel=False)       # 484
        self.gallery = self._process_dir(self.gallery_dir, os.path.join(self.mask_dir, 'gallery'), relabel=False)
    ...
    def _process_dir(self, dir_path, mask_dir, pid2label=None, relabel=False):
        ...
        for img in images:
            ...
            name = img.split('.')[0] + '.npy'
            msk_path = os.path.join(mask_dir, name)
