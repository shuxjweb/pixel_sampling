...
class CenterLossPart(nn.Module):
    """Center loss.
    ...
    def __init__(self, num_classes=751, feat_dim=2048, use_gpu=True):
        super(CenterLossPart, self).__init__()
        self.num_classes = num_classes          # 751
        self.feat_dim = feat_dim                # 2048
        self.use_gpu = use_gpu                  # False
    ...
    def forward(self, x, labels, visible):       # x->[16, 2048],  labels->[16,]
        """
        Args:
            x: feature matrix with shape (batch_size, feat_dim).
            labels: ground truth labels with shape (num_classes).
        ...
        for i in range(batch_size):
            ...
            dist.append(value)
        dist = torch.cat(dist)           # [16,]
        loss = (dist * visible).mean() / visible.mean()               # 2740.9619
        return loss
